...
//							BWAPI::Position target = u->getPosition() + BWAPI::Position(dir.x * 120, dir.y * 120);
		...
		std::cout << " episode: " << episode_num << " epsilon: " << epsilon << " sum reward | alive step | average reward " << std::endl;
		...
		os_reward << " episode " << episode_num << " epsilon " << epsilon << " sum reward | alive step | average reward " << std::endl;
		for (auto& r : set_record_rewards)
		...
		epsilon = 1.0 / (1.0 + std::sqrt(700+episode_num));
		//if (episode_num <= 1000)
