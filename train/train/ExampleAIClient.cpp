		...
		while(Broodwar->isInGame())
		{
			for(auto &e : Broodwar->getEvents())
			{
				...
				case EventType::MatchFrame:
				{
					...
					{
						//BWAPI::Broodwar->drawTextMap(u->getPosition(), "isAttackFrame()=%s", u->isAttackFrame() ? "true" : "false");
						//BWAPI::Broodwar->drawTextMap(u->getPosition() + BWAPI::Position(0, 10), "isAttacking()=%s", u->isAttacking() ? "true" : "false");
						//BWAPI::Broodwar->drawTextMap(u->getPosition() + BWAPI::Position(0, 20), "weapon cool down()=%d", u->getGroundWeaponCooldown());
					}
					...
					for (auto &u : Broodwar->self()->getUnits())
					{
						...
						{
							double last_own_hitpoint = set_last_own_hitpoints[u->getID()];
							int last_cool_down = set_last_cool_downs[u->getID()]; 
							double r = RewardAttack(u, last_own_hitpoint, last_cool_down); 
							r += RewardMove(u, set_last_states[u->getID()], set_last_actions[u->getID()]); 
							if (set_record_rewards.count(u->getID()) <= 0)
								set_record_rewards.insert(std::pair<int, double>(u->getID(), 0.0));
							set_record_rewards[u->getID()] += r;
							//std::vector<double> state = GetUnitInfoRepresent(u)-0.5;
							std::vector<double> state = GetUnitInfoRepresent(u);
							...
							if (set_Q_values.count(u->getID()) <= 0)
								set_Q_values.insert(std::pair<int, double>(u->getID(), 0.0));
							double Q1 = set_rl_agents[u->getID()].GetQValue(nn_input, action);
							set_Q_values[u->getID()] += Q1;
							double Q0 = set_last_Qs[u->getID()];
							set_temperal_diffs[u->getID()] = r + gamma*Q1 - Q0;
						}
					}
					// train net weights with eligibility
...
//							BWAPI::Position target = u->getPosition() + BWAPI::Position(dir.x * 120, dir.y * 120);
				...
				case EventType::UnitDestroy:
				{
					...
					if (set_last_Qs.count(u->getID()) > 0)
					{
						double r = RewardDestroy(u);
						if (set_record_rewards.count(u->getID()) <= 0)
							set_record_rewards.insert(std::pair<int, double>(u->getID(), 0.0));
						set_record_rewards[u->getID()] += r;
						double Q0 = set_last_Qs[u->getID()];
						set_temperal_diffs[u->getID()] = r - Q0;
					}
					// train net weights with eligibility
					// w = w+alpha*delta*eligibility
					// check if it is the first step
		...
		//std::cout << "Game ended:  episode = " << episode_num << "  epsilon = " << epsilon << "  win = " << record_win << "  lose = " << record_lose  << std::endl;
		std::cout << "episode  " << "  epsilon  " << "  win   "<< "  lose  "  << std::endl;
		std::cout << episode_num << "\t" << epsilon << "\t" << record_win << "\t" << record_lose << std::endl;
		//std::cout << "sum reward | alive step | average reward " << std::endl;
		//for (auto& r : set_record_rewards)
		//	std::cout << r.second << "\t" << set_record_steps[r.first] << "\t" << r.second / set_record_steps[r.first] << std::endl;
		//std::cout << "sum Q value | alive step | average Q value " << std::endl;
		//for (auto& q : set_Q_values)
		//	std::cout << q.second << "\t" << set_record_steps[q.first] << "\t" << q.second / set_record_steps[q.first] << std::endl;

		//os_reward << "Game ended:  episode = " << episode_num << "  epsilon = " << epsilon << "  win = " << record_win << "  lose = " << record_lose << std::endl;
		os_reward << "episode  " << "  epsilon  " << "  win   " << "  lose  " << std::endl;
		os_reward << episode_num << "\t" << epsilon << "\t" << record_win << "\t" << record_lose << std::endl;
		os_reward << "sum reward | alive step | average reward " << std::endl;
		for (auto& r : set_record_rewards)
			os_reward << r.second << "\t" << set_record_steps[r.first] << "\t" << r.second / set_record_steps[r.first] << std::endl;
		os_reward << "sum Q value | alive step | average Q value " << std::endl;
		for (auto& q : set_Q_values)
			os_reward << q.second << "\t" << set_record_steps[q.first] << "\t" << q.second / set_record_steps[q.first] << std::endl;
		...
		epsilon = 0.5 / (1.0 + std::sqrt(episode_num));
		//if (episode_num <= 1000)
		//	epsilon = 0.51 - 0.0005 * episode_num;
