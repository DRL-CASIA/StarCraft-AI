...
int main(int argc, const char* argv[])
{
	...
	std::string file_state_str = "starcraft_combat_state.txt";
	std::ofstream os_state(file_state_str, std::ofstream::out | std::ofstream::app);
	if (!os_state.is_open())
		std::ferror(NULL);
    std::string file_action_str = "starcraft_combat_action.txt"; 
	std::ofstream os_action(file_action_str, std::ofstream::out | std::ofstream::app); 
	if (!os_action.is_open())
		std::ferror(NULL); 
	/*
			...
			for(auto &e : Broodwar->getEvents())
			{
				...
				case EventType::MatchFrame:
				{
					...
					for (auto &u : Broodwar->self()->getUnits())
					{
						...
						std::vector<double> current_state = GetUnitInfoRepresent(u);
						//std::vector<double> state = GetUnitInfoRepresent(u);						
						//std::vector<int>::iterator t;
						//for (t = state.begin(); t != state.end(); t++)
						//	std::cout << *t << " ";
						size_t action; 
						...
						for (i = 0; i < current_state.size(); i++)
							os_state << current_state[i] << " ";
						os_state << std::endl;
				
						//std::cout << action << std::endl;
						os_action << action << std::endl; 
